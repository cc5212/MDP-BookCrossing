...
raw_users  = LOAD 'Datos/BX-Users.tsv' USING PigStorage('\t') AS (id, location);
raw_books  = LOAD 'Datos/BX-Books.tsv' USING PigStorage('\t') AS (isbn, title, author, year, publisher);
raw_ratings = LOAD 'Datos/BX-Book-Ratings.tsv' USING PigStorage('\t') AS (user_id, book_id, rating);
